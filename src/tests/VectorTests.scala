...
class VectorTests {
  ...
  //Testing coordinates
   ...
   //Testing minAbsolute
   /*@Test def minAbsTest() = {
     val a = new SimulationVector(5, 5)
     val mag = a.magnitude
     ...
     for(x <- -100 to 100) {
       assertEquals("larger vector was chosen instead of smaller", min(mag, vectors(i).magnitude), vectors(i).minAbsolute(a).magnitude, 0)
   ...
   @Test def averagePositionTest() = {
     ...
     vectors = Array[SimulationVector]()
     y = 0
